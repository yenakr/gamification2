  ...
  const renderParagraph = (text: string, idx: number) => {
    ...
    if (cleanText.startsWith('![') && cleanText.endsWith(')')) {
      ...
      if (lastOpenParen !== -1) {
        ...
        // 일반 표 이미지 감지 (너비를 넓게 확보해야 하는 특성 고려)
        const isTable = altText.includes('표') || srcPart.toLowerCase().includes('_table');
        ...
        const isFullWidth = isOtherImage || isTableContinuation || isTable || isAlgorithm;
